...
public class LightThingService extends AbstractVerticle {
	...
	private List<ThingAbstractAdapter<LightThingAPI>> adapters;
	
	public static final int HTTP_PORT = 8890;
	public static final int MQTT_PORT = 1890;
	
	public LightThingService(LightThingAPI model) {
		this.model = model;
		adapters = new LinkedList<ThingAbstractAdapter<LightThingAPI>>();
	}
	...
	protected void installAdapters(Promise<Void> promise) {		
		...
		try {
			 ...
			 */
			LightThingHTTPAdapter httpAdapter = new LightThingHTTPAdapter(model, "localhost", HTTP_PORT, this.getVertx());
		...
		try {
			 ...
			 */
			LightPAShadMQTT shad = new LightPAShadMQTT(model, "MyLampDT","localhost", MQTT_PORT, this.getVertx());
			Promise<Void> p = Promise.promise();
			shad.setupAdapter(p);
			Future<Void> fut = p.future();
			allFutures.add(fut);
	...
	protected void log(String msg) {
		System.out.println("[LightThingService] " + msg);
	}
}
